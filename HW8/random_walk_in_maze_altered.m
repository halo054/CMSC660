function random_walk_in_maze()
close all
%% load data
data_down = readmatrix("maze_data_down.csv");
data_right = readmatrix("maze_data_right.csv");
data_down(1,:) = [];
data_down(:,1) = [];
data_right(1,:) = [];
data_right(:,1) = [];
%% draw maze
fig = 1;
draw_maze(data_down,data_right,fig)
% each maze cell is a node
% generate adjacency matrix
[m,n] = size(data_down);
N = m*n;
A = make_adjacency_matrix(data_down,data_right);
%% convert adjacency matrix to stochastic matrix 
row_sums = sum(A,2); % find rowsums of A
R = spdiags(1./row_sums,0,N,N);
P = R*A;
%% setup linear system for the committor problem
exitA = 1;
exitB = N;
x = zeros(N,1);
x(exitB) = 1;
I = speye(N);
L = P - I;
b = -L*x;
ind_exits = union(exitA,exitB);
ind_unknown = setdiff((1:N),ind_exits);
%% make the system symmetric positive definite
% L = R^{-1}A - I
% R^{1/2}LR^{-1/2} = R^{1/2}AR^{1/2} - I is symmetric
% Lsymm = R^{1/2}LR^{-1/2}, bsymm = R^{1/2}*b
% D = R^{1/2}
r_sqrt = sqrt(row_sums);
D = spdiags(r_sqrt,0,N,N);
Dinv = spdiags(1./r_sqrt,0,N,N);
Lsymm = D*L*Dinv;
bsymm = D*b;




%% (*)use writematrix to export Lsymm and bsymm into python

Matrix_Lsymm = full(Lsymm);
Matrix_bsymm = full(bsymm);
writematrix(Matrix_Lsymm,"Lsymm.csv")
writematrix(Matrix_bsymm,"bsymm.csv")

%% (*)calculate incomplete cholesky and export the preconditioning Matrix into python
ichol_fac = ichol(sparse(-Lsymm(ind_unknown,ind_unknown)));
M = ichol_fac*ichol_fac';
Matrix_M = full(M);
writematrix(Matrix_M,"M.csv");


%% solve the linear system L*x(ind_unknown) = b
x(ind_unknown) = Lsymm(ind_unknown,ind_unknown)\bsymm(ind_unknown);
solution = x(ind_unknown);

% (*) result from python are loaded back into matlab for visualization
x_without_preconditioning = zeros(N,1);
x_without_preconditioning(exitB) = 1;
x_sliced_without_preconditioning = readmatrix("x_sliced.csv");

x_with_preconditioning = zeros(N,1);
x_with_preconditioning(exitB) = 1;
x_sliced_with_preconditioning = readmatrix("x_sliced_2.csv");

x_without_preconditioning(ind_unknown) = Dinv(ind_unknown,ind_unknown)*x_sliced_without_preconditioning;
x_with_preconditioning(ind_unknown) = Dinv(ind_unknown,ind_unknown)*x_sliced_with_preconditioning;

%solution = full(solution);
%writematrix(solution,"solution.csv");
%writematrix(x,"x.csv");
%writematrix(x(ind_unknown),"x(ind_unknown).csv");

x(ind_unknown) = Dinv(ind_unknown,ind_unknown)*x(ind_unknown);
%% visualize the solution
figure(3);
committor = reshape(x,[m,n]);
imagesc(committor);
draw_maze(data_down,data_right,3)

% (*)figure 5 and 6 are result by CG imported from python
figure(5);
committor_without_preconditioning = reshape(x_without_preconditioning,[m,n]);
imagesc(committor_without_preconditioning);
draw_maze(data_down,data_right,5)

figure(6);
committor_with_preconditioning = reshape(x_with_preconditioning,[m,n]);
imagesc(committor_with_preconditioning);
draw_maze(data_down,data_right,6)
%% find eigenvalues of Lsymm
evals = sort(real(eig(full(-(Lsymm)))),'descend');
figure(4);
plot(evals,'.','Markersize',15);
grid on
set(gca,'fontsize',20)
xlabel('eigenvalue index','FontSize',20);
ylabel('eigenvalue','FontSize',20);
%% save the solution
A = -Lsymm(ind_unknown,ind_unknown);
b = -bsymm(ind_unknown);
save("maze_linear_system.mat","A","b","solution");

end

%%
function draw_maze(data_down,data_right,fig)
[m,n] = size(data_down);
figure(fig);
hold on;
line_width = 3;
col = 'k';
% plot outer lines
plot(0.5+(1:n),0.5+zeros(1,n),'color',col,'Linewidth',line_width);
plot(0.5+(0:n-1),0.5+m*ones(1,n),'color',col,'Linewidth',line_width);
plot(0.5+zeros(1,m),0.5+(1:m),'color',col,'Linewidth',line_width);
plot(0.5+m*ones(1,n),0.5+(0:m-1),'color',col,'Linewidth',line_width);
% plot vertical lines
for i = 1 : m
    for j = 1 : n-1
        if data_right(i,j) == 0
            plot(0.5+[j,j],0.5+[i-1,i],'color',col,'Linewidth',line_width);
        end
    end
end
% plot horizontal lines
for j = 1 : n
    for i = 1 : m-1
        if data_down(i,j) == 0
            plot(0.5+[j-1,j],0.5+[i,i],'color',col,'Linewidth',line_width);
        end
    end
end
axis ij
axis off
daspect([1,1,1])
end
%%
function A = make_adjacency_matrix(data_down,data_right)
[m,n] = size(data_down);
mn = m*n;
A = sparse(mn);
for i = 1 : m
    for j = 1 : n-1
        if data_right(i,j) == 1
            ind = (j-1)*m + i;
            A(ind,ind+m) = 1;
            A(ind+m,ind) = 1;
        end
    end
end
for j = 1 : n
    for i = 1 : m-1
        if data_down(i,j) == 1
            ind = (j-1)*m + i;
            A(ind,ind+1) = 1;
            A(ind+1,ind) = 1;
        end
    end
end
end



